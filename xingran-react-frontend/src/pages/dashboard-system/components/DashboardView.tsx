...
} from "@/components/dashboard/layout/DashboardGrid";
...
export const DashboardView: React.FC<DashboardViewProps> = ({ dashboardId, isHome = false }) => {
  ...
  if (currentLoading) {
    return (
      <div
        className="dashboard-view loading"
  ...
  if (!currentDashboard) {
    return (
      <div
        className="dashboard-view empty"
        style={{ display: "flex", justifyContent: "center", alignItems: "center", height: "400px" }}
      >
        <p>仪表盘不存在</p>
      </div>
    ...
    <Space style={{ marginBottom: isHome ? 16 : 0 }}>
      ...
      {isHome ? (
        ...
        <Button
          type="primary"
          icon={<EditOutlined />}
          onClick={() => navigate(`${DASHBOARD}/${dashboardId}?mode=edit`)}
        >
          编辑仪表盘
        </Button>
      ) : (
