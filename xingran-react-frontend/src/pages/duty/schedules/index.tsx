  ...
  return [
    ...
    },
    {
      ...
      render: (_: unknown, record: DutySchedule) => {
        ...
        return isExpired ? <Tag color="red">已过期</Tag> : <Tag color="green">未过期</Tag>;
      },
    },
    {
      title: "状态",
...
const DutySchedulePage: FC = () => {
  ...
  // 表格列
  const columns = getDutyTableColumns({
    current: paginationProps.current ?? 1,
    pageSize: paginationProps.pageSize ?? 10,
    handleDelete,
    getColumnSortOrder,
    ...
    <div className="p-6">
      ...
      <Card
        ...
        extra={
          <Space>
            ...
            <Button onClick={handleTodayWeek}>本周</Button>
      ...
      >
        <Form form={swapForm} layout="vertical" preserve={false}>
          ...
          >
            <Select
              placeholder="请选择原排班记录"
              showSearch
              optionFilterProp="label"
              onSearch={() => {}}
            >
              {allSchedules.map((s) => (
                <Option key={s.id} value={s.id} label={formatScheduleOptionLabel(s)}>
