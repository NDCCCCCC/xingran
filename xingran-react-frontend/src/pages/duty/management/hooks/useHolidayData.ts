...
export function useHolidayData() {
  ...
  // 创建节假日
  const create = useCallback(async (data: Omit<Holiday, "id" | "createdAt" | "updatedAt" | "createdBy">) => {
    try {
  ...
  // 更新节假日
  const update = useCallback(async (id: string, data: Partial<Holiday>) => {
    try {
  ...
  // 批量创建节假日
  const batchCreate = useCallback(async (dataList: Array<{
    holidayDate: string;
    holidayName: string;
    isOffday: boolean;
