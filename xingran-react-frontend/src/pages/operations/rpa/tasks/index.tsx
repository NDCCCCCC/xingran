...
const TaskManagement: FC = () => {
  ...
  const { orderByColumn, isAsc, handleTableChange: handleTaskSortChange, sortOrder: taskSortOrder } = useServerSort<Task>({
    sorterMetas,
  });
  ...
  const handleExecute = useCallback(async (id: string, name: string) => {
    Modal.confirm({
      title: "确认执行",
  ...
  const columns = useMemo(
    () =>
      getTaskColumns({
        ...
        handleDelete,
        handleExecute,
        getSortOrder: (field) => (orderByColumn === field ? (taskSortOrder ?? null) as "ascend" | "descend" | null : null),
      }),
