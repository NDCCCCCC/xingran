...
const BuildingManagement: FC = () => {
  ...
  const {
    ...
    handleReset: _handleReset, handleAdd, handleEdit, handleModalClose, loadData: loadBuildings, resetSelection,
    getColumnSortOrder,
    handleTableChange,
  } = useTableManager<Building>(
    async (params) => {
      const result = await buildingApi.list(params) as { data?: { list: Building[]; total: number } };
  ...
  const loadStatistics = useCallback(async (searchParams: Record<string, unknown> = {}) => {
    ...
    } catch (error) { handleApiError(error, "加载统计数据", false); }
  }, []);
  ...
  const handleDelete = async (id: string) => {
    try {
      ...
      loadBuildings(searchParams);
      loadStatistics(searchParams);
    } catch (error) { handleApiError(error, "删除"); }
  };
  ...
  const handleBatchDelete = async () => {
    ...
    try {
      ...
      loadBuildings(searchParams);
      loadStatistics(searchParams);
    } catch (error) { handleApiError(error, "批量删除"); }
  };
  ...
  const columns: ColumnsType<Building> = [
    { title: "楼宇名称", dataIndex: "name", key: "name", width: 150, sorter: true, sortOrder: getColumnSortOrder("name") },
    { title: "所属机构", dataIndex: "orgId", key: "orgName", width: 150, sorter: true, sortOrder: getColumnSortOrder("orgId"), render: getOrgName },
    {
      title: "层级",
      dataIndex: "level",
      key: "level",
      ...
      render: (level: number) => (
        <Tag color={level === 1 ? "purple" : "blue"}>
          {level === 1 ? "一级" : "二级"}
        </Tag>
      ),
    },
    { title: "地址", dataIndex: "address", key: "address", ellipsis: true, width: 200, sorter: true, sortOrder: getColumnSortOrder("address") },
    {
      title: "经纬度",
      key: "coordinates",
      width: 150,
    ...
    { title: "楼层数", dataIndex: "totalFloors", key: "totalFloors", width: 100, sorter: true, sortOrder: getColumnSortOrder("totalFloors"), render: (v) => v || 0 },
    ...
    createDateTimeColumn("createdAt", { width: 180, title: "创建时间", sorter: true, sortOrder: getColumnSortOrder("createdAt") }),
  ...
  const renderCardView = () => {
    if (buildings.length === 0) return <div style={{ textAlign: "center", padding: "40px", color: "var(--theme-text-tertiary, #999)" }}>暂无数据</div>;
    return (
      <Row gutter={[16, 16]}>
              ...
              <Card.Meta
                title={
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    ...
                    <Tag color={building.status === 0 ? "success" : "error"}>{building.status === 0 ? "正常" : "1"}</Tag>
                  </div>
                }
                description={
                  <div>
                    {building.address && <div><strong>地址：</strong>{building.address}</div>}
                    <div><strong>所属机构：</strong>{getOrgName(building.orgId)}</div>
                    <div><strong>楼层：</strong>{building.totalFloors || 0}层</div>
                  </div>
                }
              />
            </Card>
  ...
  return (
    <Layout style={{ background: "#000", minHeight: "calc(100vh - 64px)" }}>
      ...
      <Content style={{ background: "#fff" }}>
        <div>
          <Row gutter={16} style={{ marginBottom: 16 }}>
            <Col span={8}><Card><Statistic title="总楼宇数" value={statistics.total} prefix={<CheckCircleOutlined />} /></Card></Col>
            <Col span={8}><Card><Statistic title="正常楼宇" value={statistics.active} styles={{ content: { color: "var(--theme-success, #3f8600)" } }} prefix={<CheckCircleOutlined />} /></Card></Col>
            <Col span={8}><Card><Statistic title="停用楼宇" value={statistics.inactive} styles={{ content: { color: "var(--theme-error, #cf1322)" } }} prefix={<StopOutlined />} /></Card></Col>
          </Row>
          <Card style={{ marginBottom: 16 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: "16px" }}>
          <Form form={searchForm} layout="inline" style={{ flex: 1, minWidth: 0 }}>
            <Form.Item name="name" label="楼宇名称"><Input placeholder="请输入楼宇名称" allowClear className="user-form-input" style={{ width: 150 }} /></Form.Item>
            <Form.Item name="status" label="状态">
              <Select placeholder="请选择状态" allowClear className="user-form-input" style={{ width: 120 }} onSearch={() => {}}><Option value={0}>正常</Option><Option value={1}>停用</Option></Select>
            </Form.Item>
            <Form.Item>
              <Space><Button type="primary" icon={<SearchOutlined />} onClick={handleSearch}>搜索</Button><Button onClick={handleResetWithDept}>重置</Button><Button icon={<ReloadOutlined />} onClick={handleRefresh}>刷新</Button></Space>
            </Form.Item>
          </Form>
          <Space>
            ...
            <Button type="primary" icon={<PlusOutlined />} onClick={() => openModal()}>新增楼宇</Button>
          </Space>
        </div>
        {selectedRowKeys.length > 0 && <Alert title={<span>已选择 <strong>{selectedRowKeys.length}</strong> 个楼宇，<Button type="link" size="small" onClick={() => setSelectedRowKeys([])} style={{ padding: 0 }}>取消选择</Button></span>} type="info" showIcon style={{ marginTop: 12 }} />}
      </Card>
      <Card>
        {viewMode === "table" ? (
          <Table
      ...
      >
        <Form form={buildingForm} layout="horizontal" labelCol={{ span: 5 }} wrapperCol={{ span: 19 }}>
          ...
          <Form.Item name="totalFloors" label="楼层数">
            <InputNumber min={0} disabled style={{ width: "100%" }} placeholder="根据创建的楼层自动计算" />
          </Form.Item>
      ...
      <ExcelImport entityType="building" entityName="楼宇" visible={importVisible} onClose={() => setImportVisible(false)} onImportSuccess={handleImportSuccess} />
      <ExcelExport entityType="building" entityName="楼宇" visible={exportVisible} onClose={() => setExportVisible(false)} filters={exportFilters} />
        </div>
      </Content>
    </Layout>
  );
