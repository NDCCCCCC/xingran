...
const DedicatedLineManagement: FC = () => {
  ...
  const loadServerRooms = useCallback(async () => {
    try {
      const result = (await serverRoomApi.list({ current: 1, pageSize: 50 })) as {
  ...
  const {
    ...
    async (params) => {
      const result = (await dedicatedLineApi.list(params)) as {
        data?: { list: DedicatedLine[]; total: number };
      };
  ...
  // 初始化加载
  useEffect(() => {
    Promise.all([loadServerRooms(), loadStatistics()]).then(([_, stats]) => setStatistics(stats));
    loadLines();
  ...
  const columns: ColumnsType<DedicatedLine> = [
    {
      ...
      width: 150,
      sorter: true,
      sortOrder: getLineColumnSortOrder("name"),
    },
    {
      title: "专线类型",
      dataIndex: "lineType",
    ...
    },
    {
      ...
      width: 100,
      sorter: true,
      sortOrder: getLineColumnSortOrder("bandwidth"),
      render: (v) => v || "-",
    },
    {
      title: "运营商",
      ...
      key: "isp",
      width: 120,
      render: (isp) => <Tag>{getIspText(isp as string)}</Tag>,
    },
    {
      title: "源机房",
      dataIndex: "sourceRoomName",
      key: "sourceRoomName",
      width: 120,
      render: (v) => v || "-",
    },
    ...
    createDateTimeColumn("createdAt", {
      width: 180,
      sorter: true,
      sortOrder: getLineColumnSortOrder("createdAt"),
    }),
    {
      title: "操作",
      key: "action",
              ...
              <Card.Meta
                title={
                  ...
                  >
                    ...
                    <Tag
                      color={
                        line.status === 0 ? "success" : line.status === 1 ? "error" : "default"
                      }
                    >
        ...
        <Card style={{ marginBottom: 16 }}>
          ...
          {selectedRowKeys.length > 0 && (
            <Alert
              title={
                <span>
                  已选择 <strong>{selectedRowKeys.length}</strong> 条专线，
                  <Button
                    type="link"
                    size="small"
        ...
        >
          ...
          >
            <Form.Item
              name="name"
              label="专线名称"
              rules={[{ required: true, message: "请输入专线名称" }]}
            >
            ...
            <Form.Item
              name="lineType"
              label="专线类型"
              rules={[{ required: true, message: "请选择专线类型" }]}
            >
