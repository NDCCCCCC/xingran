...
export const roomDeviceApi = {
  ...
  // 更新
  update: (id: string, data: Partial<RoomDevice>) => post(`/ops/room-devices/${id}/update`, data),

  // 删除
  delete: (id: string) => post(`/ops/room-devices/${id}/delete`),

  // 批量删除
  batchDelete: (ids: string[]) => post("/ops/room-devices/batch", { ids, action: "delete" }),

  // 导出
  export: (params: PageParams) => post("/ops/room-devices/export", params),
  ...
  // 下载模板
  downloadTemplate: () => post("/ops/room-devices/template", {}),
};
