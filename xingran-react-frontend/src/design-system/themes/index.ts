...
function applyColorVariables(colors: ThemeConfig["colors"]): void {
  ...
  if (colors.primary[0]) {
    ...
    root.style.setProperty(
      "--theme-primary-600",
      colors.primary[3] || colors.primary[2] || colors.primary[0]
    );
    root.style.setProperty(
      "--theme-primary-700",
      colors.primary[4] || colors.primary[3] || colors.primary[0]
    );
  }
  ...
  const brandColor =
    colors.accent?.[1] || colors.secondary?.[3] || colors.primary?.[2] || DEFAULT_COLOR;
  const brandColorDark =
    colors.accent?.[2] || colors.secondary?.[4] || colors.primary?.[3] || "#2563eb";
