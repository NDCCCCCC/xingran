...
const ColorSwitcher: FC<ColorSwitcherProps> = ({
	size = "middle",
	type = "dropdown",
}) => {
	...
	// 确保颜色值始终是字符串格式
	const primaryColor = (typeof preferences.theme.customColors?.primary === "string"
		? preferences.theme.customColors.primary
		: "#4F46E5") as string;
	const sidebarColor = (typeof preferences.theme.customColors?.sidebar === "string"
	...
	// 初始化：如果有自定义颜色，应用它们
	useEffect(() => {
		if (preferences.theme.customColors?.primary && typeof preferences.theme.customColors.primary === "string") {
	...
	const getCurrentColorName = () => {
		const preset = primaryColorPresets.find(p => p.primary.toLowerCase() === primaryColor.toLowerCase());
	...
	const getSidebarColorName = () => {
		const preset = sidebarColorPresets.find(p => p.primary.toLowerCase() === sidebarColor.toLowerCase());
		if (preset) return preset.name;
		return "自定义";
	};
	...
	const menuItems: MenuProps["items"] = [
		...
		// 侧边栏颜色选择
		{
			...
			label: (
				<div style={{ padding: "8px 0" }}>
					<Space>
						<MenuOutlined style={{ color: getLuminance(sidebarColor) < 0.5 ? sidebarColor : "#8B5CF6" }} />
		...
		},
		{
			type: "group",
			label: "深色背景（配浅色文字）",
			children: sidebarColorPresets.filter(p => getLuminance(p.primary) < 0.5).map((preset) => ({
