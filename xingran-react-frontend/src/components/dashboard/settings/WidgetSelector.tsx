...
export const WidgetSelector: React.FC<WidgetSelectorProps> = ({
  ...
  const handleConfirm = async () => {
    try {
      ...
      const finalDisplayConfig: DisplayConfig =
        displayConfig || createDefaultDisplayConfig(values.type as WidgetType);

      const widget: WidgetConfig = {
        id:
          editingWidgetId || `widget-${Date.now()}-${Math.random().toString(36).substring(2, 11)}`,
        type: values.type,
        title: values.title,
        ...
        {!selectedType ? (
          ...
          <div className="widget-selector__config">
            ...
            <Form form={form} layout="vertical" initialValues={{ method: "GET" }}>
              ...
              <Form.Item label="Widget类型" name="type" rules={[{ required: true }]}>
