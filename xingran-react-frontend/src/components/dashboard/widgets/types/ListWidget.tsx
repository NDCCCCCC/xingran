...
export const ListWidget: React.FC<ListWidgetProps> = ({ widget, display, onEdit, onDelete }) => {
        ...
        renderItem={(item, index) => (
          <List.Item key={String((item as Record<string, unknown>).id ?? index)}>
            <div className="list-widget-item">
              {display.showIndex && <span className="list-widget-item__index">{index + 1}.</span>}
              {display.iconField && (
              ...
              {display.timeField && (
                <div className="list-widget-item__time">
                  <ClockCircleOutlined />
                  <span>
                    {formatTime((item as Record<string, unknown>)[display.timeField] as string)}
                  </span>
                </div>
              )}
            </div>
