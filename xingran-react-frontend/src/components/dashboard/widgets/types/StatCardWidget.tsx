	...
	const { value, label, trend, icon, color } = useMemo(() => {
		...
		if (numericValue === undefined || numericValue === null) {
			const possibleFields = [
				"totalDevices", "onlineDevices", "offlineDevices", "unknownDevices",
				"deviceCount", "serverCount", "userCount", "orderCount",
	...
	return (
		<div className="stat-card-widget" style={{ color }}>
			...
			{!loading && !error && (
				<>
					{icon && (
						<div className="stat-card-widget__icon" style={{ backgroundColor: color ? `${color}20` : undefined }}>
