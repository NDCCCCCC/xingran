  ...
  const { percent, color, status } = useMemo(() => {
    if (!data || typeof data !== "object") {
      return {
        percent: 0,
        color: undefined,
        status: undefined as "success" | "exception" | "normal" | undefined,
      };
    }
  ...
  const multiple = useMemo(() => {
    ...
    return items.map((item, index) => {
      ...
      return {
        ...
        percent: Math.min(
          Math.round((Number(i.value ?? 0) / (Number(i.target ?? 100) || 1)) * 100),
          100
        ),
    ...
    >
      <div className="progress-widget">
        {display.progressType === "circle" ? (
          ...
          <Progress percent={percent} strokeColor={color} status={status} />
        )}
      </div>
    </BaseWidget>
  );
