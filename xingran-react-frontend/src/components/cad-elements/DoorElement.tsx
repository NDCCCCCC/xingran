...
export const DoorElement = React.memo(
  ({
    door,
    selected = false,
    hovered = false,
    ...
    const geometry = useMemo<DoorGeometry>(() => {
      ...
      const hingePoint = {
        x: snapCoord(direction === "left" ? position.x + cos * halfWidth : position.x - cos * halfWidth),
        y: snapCoord(direction === "left" ? position.y + sin * halfWidth : position.y - sin * halfWidth),
      };
