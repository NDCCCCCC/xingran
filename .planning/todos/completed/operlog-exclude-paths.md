...
**已实现确认关闭**——机制由前序会话完整落地（代码注释标记 Phase 35 OPERLOG-01）：

- `internal/utils/operlog/operlog.go`：`ExcludedPaths` + `Configure()` + `IsExcludedPath()`（filepath.Match + `/*` 后缀通配，与 request_decryption 同语义）；`Record`/`RecordWithBody` 顶部早返回守卫（RecordWithBody 在 `GetRawData` 之前，满足 todo 关键约束）
- `internal/config/config.go`：`OperLogConfig.ExcludePaths`（mapstructure `operlog.exclude_paths`）
- `internal/core/core.go:146`：启动期 `operlog.Configure(cfg.OperLog.ExcludePaths)` 引导
- 测试：`operlog_test.go:254 TestIsExcludedPath` + `regression_test.go:306 TestExcludedPathsEarlyReturn`
- 配置：`config.prod.example.yaml` 默认含 `/api/v1/rpa/workers/*/heartbeat`；`config.yaml`/`config.example.yaml` 刻意保持空并在注释中说明 dev 保留心跳审计便于本地排查（附对齐生产的切换说明）——设计决策非缺口

未执行项：todo Step 5 的 5 分钟真实 PG + worker 实测（本会话无运行环境）；匹配逻辑已由上述测试锁定。
