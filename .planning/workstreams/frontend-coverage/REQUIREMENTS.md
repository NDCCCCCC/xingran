...
- [ ] **GOV-05**: per-directory floor gate（白名单外目录 ≥70%，未达标目录走白名单 ratchet 过渡，对称后端 `p1_package_check` / `p2_package_check`）
...
- [ ] **INFRA-02**: utils 层 ≥70%（sm2 / sm4 / encoding 国密、token 三件套、dualLevelCache、errorHandler、authHelpers、deptUtils、datetime 等 23 文件 950 stmts）
